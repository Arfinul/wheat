...
import cv2, numpy as np, random, os, pickle, keras
from train_utils.mlp import train
from train_utils.PCA import pca
from segmentation.util import get_boundry_img_matrix, get_files
...
EIGEN_VALUE_2 = 6
ECCENTRICITY = 7
features = {0: 'MEAN_AREA', 1: 'PERIMETER', 2: 'R', 3: 'B', 4: 'G', 5: 'EIGEN_VALUE_1', 6: 'EIGEN_VALUE_2',
            7: 'ECCENTRICITY', 8: 'NUMBER_GRAIN', }
#########################################

data_dir = './wheat_train_dataset/'
result_dir = './model_2_class/'
...
if __name__ == "__main__":
    ...
    if os.path.isfile(feat_data):
        ...
        grain_partical_list['not_grain'] = get_files(grain_particles['damaged_grain']) + get_files(
            grain_particles['foreign']) + get_files(grain_particles['broken_grain']) + get_files(
            grain_particles['grain_cover'])
        ...
        for gi in range(len(xctrain)):
            ...
            mean_area = area / (h * w)
            r, b, g = np.sum([gcolor[i, j] for j in range(gcolor.shape[1]) for i in range(gcolor.shape[0])], axis=0) / (
                        area * 256)
