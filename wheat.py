...
from segmentation.util import display_mask, get_boundry_img_matrix
from train_utils.PCA import pca
import numpy as np, keras, cv2
from segmentation.segment_formation import segment_image
...
if __name__ == "__main__":
    ...
    model = keras.models.load_model('model_2_class/weights_01234567.pkl')
    ...
    h, w, _ = rect.shape
    cv2.putText(rect, text='Number of good grain: %d  Number Not good grain or imputity: %d' % (good, not_good),
                org=(10, h - 50), fontScale=1, fontFace=font, color=(255, 255, 255), thickness=2, lineType=cv2.LINE_AA)
