...
def segment_image4(img_file):
    ...
    gray = np.array([[max(pixel - 25, 0) if pixel < T else min(pixel + 25, 255) for pixel in row] for row in gray],
                    dtype=np.uint8)
    ...
    for sindex in s_range:
        if s[sindex][0] - s[sindex][1] and s[sindex][2] - s[sindex][3]:
            iimg = np.array(
                [[img[i, j] if mask[i, j] == sindex else [0, 0, 0] for j in range(s[sindex][2], s[sindex][3])] for i in
                 range(s[sindex][0], s[sindex][1])], dtype=np.uint8)
            if len(iimg) == 0:
                continue
            ...
            ######################################## segmenting adding ########################
            m = s.pop(sindex)
            mask[m[0]:m[1], m[2]:m[3]] = [[0 if pixel == sindex else pixel for pixel in row] for row in
    ...
    for i in s:
        ...
        segments[i] = np.array([[org[x, y] if mask[x, y] == i else [0, 0, 0] for y in range(s[i][2], s[i][3])] for x in
                                range(s[i][0], s[i][1])], dtype=np.uint8)
