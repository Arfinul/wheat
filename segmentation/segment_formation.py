...
import keras
from util import *
from level_2 import L2_segmentation_2
import time

mFile = 'model_segmentation/weights_30_30_.h5'
...
def get_img_value_inRange(img, mask, sindex, s):
    return np.array(
        [[img[i, j] if mask[i, j] == sindex else [0, 0, 0] for j in range(s[2], s[3])] for i in range(s[0], s[1])],
        dtype=np.uint8)


def remove_mask(mask, val, mrange):
    mask[mrange[0]:mrange[1], mrange[2]:mrange[3]] = [[0 if pixel == val else pixel for pixel in row] for row in
                                                      mask[mrange[0]:mrange[1], mrange[2]:mrange[3]]]
    return mask
...
def segment_image(img_file, dlog=0):
    ...
    mask = get_8connected(thresh, mcount=5)
